...
export default function FloatingContact() {
  const [isExpanded, setIsExpanded] = useState(() => {
    try {
      return sessionStorage.getItem("floating_contact_collapsed") !== "true";
    } catch {
      return true;
    }
  });
  ...
  const toggleExpand = () => {
    setIsExpanded((prev) => {
      const next = !prev;
      try {
        sessionStorage.setItem("floating_contact_collapsed", String(!next));
      } catch {}
      return next;
    });
  };
  ...
  return (
    <aside
      ...
      className="fixed right-2.5 bottom-[76px] z-40 flex flex-col items-center select-none pointer-events-none"
    >
      {/* Danh sách 3 nút liên hệ (Messenger, Zalo, Hotline) */}
      <div
        className={`flex flex-col items-center gap-1.5 transition-all duration-300 origin-bottom ${
          isExpanded
            ? "opacity-100 scale-100 max-h-[260px] pointer-events-auto mb-1.5"
            : "opacity-0 scale-75 max-h-0 pointer-events-none overflow-hidden -translate-y-2 mb-0"
        }`}
      >
        {/* 1. Messenger Button */}
      ...
      {/* 4. Nút Thu gọn / Mở rộng (Toggle Button) */}
      <div className="relative flex items-center justify-center pointer-events-auto">
        {isExpanded ? (
          /* Khi đang mở: Nút tròn nhỏ gọn với mũi tên thu gọn xuống */
          <button
            type="button"
          ...
          /* Khi đang thu gọn: Nút liên hệ tinh gọn với icon hỗ trợ và mũi tên mở rộng lên */
